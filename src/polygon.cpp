...
bool Rect::isIntersecting(const Rect& a, const Rect& b)
{
	...
	// The rectangles overlap on both axes, so they must intersect
	else
		return true;
}
...
// Not including adjacent edges (vertex joints and possibly adjacent collinear edge overlap)
...
std::vector<std::pair<Vec, Vec>> Polygon::getEdges(const std::vector<Vec>& vertices)
{
	...
	for (int i = 0; i < edges.size() - 1; ++i)
	{
		...
		edges[i].second = vertices[i + 1] - vertices[i];
	}

	// Set last edge
	edges.back().first = vertices.back();
	edges.back().second = vertices.front() - vertices.back();
...
* Partitions a polygon into triangles (starting from origin)
* Finds area and centroid of each triangle
* Multiplies the two together to get a vector for each triangle
...
void Polygon::initPos()
{
	...
	// Sum the area and centroid of each resulting triangle  
	auto addWeightedCentroid = [&](const Vec& 𝙫0, const Vec& 𝙫1)
	{
		float triangleArea = 𝙫0.cross(𝙫1);    // Twice the area of a shoelace triangle, can be signed
	...
	// Add last triangle
	addWeightedCentroid(mAbsoluteVertices.back(), mAbsoluteVertices.front());
...
Vec ConvexPolygon::resolveCollisions(const ConvexPolygon& moveableShape, const std::vector<ConvexPolygon>& fixedShapes)
{
	...
	{
		// Get every collision solution between the movable and fixed shapes
...
// Use separating axis theorem to find a resolution vector
Vec ConvexPolygon::resolveCollision(const ConvexPolygon& moveableShape, const ConvexPolygon& fixedShape)
{
	...
	for (auto& axis : axi)
	{
		...
		// Scalar projecting on to a normalized vector simplifies to the dot product
		...
		// Find overlap, always positive, regardless of axis / vertex direction
	...
	Vec direction = moveableShape.getPos() - fixedShape.getPos();   // Get vector that points from fixed to movable shape
...
std::vector<Vec> ConvexPolygon::getCollisionAxi() const
{
	...
	// Get last axis (edge going from last to first vertices)
	edges.back() = vertices.front() - vertices.back();
	edges.back() *= Matrix{ 0, -1, 1, 0 };
	edges.back().normalize();
...
/*
* Implementation:
* SVG files do not guarantee the order of the vertices,
